...
function add-user() {
	...
	echo -e "$user\t$exp" >> /multivpn/wireguard/wireguard-clients.txt
	echo -e "[Interface]
...
Endpoint = ${endpoint}
AllowedIPs = 0.0.0.0/0" >> /multivpn/wireguard/${user}.conf
	echo -e "\n### Client ${user}
[Peer]
...
function delete-user(){
	...
	if grep -qw "^### Client ${user}\$" /etc/wireguard/wg0.conf; then
		...
		rm -f /multivpn/wireguard/${user}.conf
		sed -i "/\b$user\b/d" /multivpn/wireguard/wireguard-clients.txt
		systemctl daemon-reload
		systemctl restart wg-quick@wg0
		echo -e ""
...
function extend-user() {
	...
	echo -e "---------------------"
	read -p "Username : " user
	if ! grep -qw "$user" /multivpn/wireguard/wireguard-clients.txt; then
		echo -e ""
		echo -e "User '$user' does not exist."
		echo -e ""
		exit 0
	...
	exp_old=$(cat /multivpn/wireguard/wireguard-clients.txt | grep -w $user | awk '{print $2}')
	...
	sed -i "/\b$user\b/d" /multivpn/wireguard/wireguard-clients.txt
	echo -e "$user\t$exp_new" >> /multivpn/wireguard/wireguard-clients.txt
...
function user-list() {
	...
	do
		...
		printf "%-17s %2s\n" "$user" "$exp_date"
	done < /multivpn/wireguard/wireguard-clients.txt
	total=$(wc -l /multivpn/wireguard/wireguard-clients.txt | awk '{print $1}')
	echo -e "-------------------------------"
...
function show-config() {
	...
	read -p "User : " user
	if grep -qw "^### Client ${user}\$" /etc/wireguard/wg0.conf; then
		exp=$(cat /multivpn/wireguard/wireguard-clients.txt | grep -w "$user" | awk '{print $2}')
		exp_date=$(date -d"${exp}" "+%d %b %Y")
		echo -e "Expired : $exp_date"
		echo -e ""
		echo -e "QR Code"
		echo -e "-------"
		qrencode -t ansiutf8 -l L < /multivpn/wireguard/${user}.conf
		echo -e ""
		echo -e "Config"
		echo -e "------"
		cat /multivpn/wireguard/${user}.conf
		echo -e ""
	else
		echo -e ""
		echo -e "User '$user' does not exist."
