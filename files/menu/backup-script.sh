...
startBackup() {
	...
	if gdrive list | grep -qw "vpn backup"; then
		...
		gdrive mkdir "vpn backup" --description "VPN account backup folder. Script by Multi VPN." > /dev/null 2>&1
		folderID=$(gdrive list | grep -w "vpn backup" | awk '{print $1}')
	else
		gdrive mkdir "vpn backup" --description "VPN account backup folder. Script by Multi VPN." > /dev/null 2>&1
		folderID=$(gdrive list | grep -w "vpn backup" | awk '{print $1}')
	...
	rm -rf /backup-vpn
	mkdir /backup-vpn
	cp /multivpn/xray/xray-clients.txt /backup-vpn/
	cp /multivpn/ssh/ssh-clients.txt /backup-vpn/
	...
	if ! [[ -s xray-clients.txt ]]; then
		...
		while read account; do
			...
			if grep -qw "$user" /multivpn/xray/xray-clients.txt; then
				...
				if grep -qw "$uuid" /multivpn/xray/xray-clients.txt; then
	...
	if ! [[ -s ssh-clients.txt ]]; then
		...
		while read account; do
			...
			if getent passwd $user > /dev/null 2>&1; then
				...
				echo -e "${user}\t${pass}\t${exp}" >> /multivpn/ssh/ssh-clients.txt
