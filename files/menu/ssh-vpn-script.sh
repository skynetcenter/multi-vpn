...
function add-user() {
	...
	echo -e "${user}\t${pass}\t$(date -d +${duration}days +%Y-%m-%d)" >> /multivpn/ssh/ssh-clients.txt
...
function delete-user() {
	...
	if getent passwd $user > /dev/null 2>&1; then
		userdel $user
		sed -i "/\b$user\b/d" /multivpn/ssh/ssh-clients.txt
		echo -e ""
...
function show-information() {
	...
	read -p "User : " user
	if getent passwd $user > /dev/null 2>&1; then
		pass=$(cat /multivpn/ssh/ssh-clients.txt | grep -w "$user" | awk '{print $2}')
		exp=$(cat /multivpn/ssh/ssh-clients.txt | grep -w "$user" | awk '{print $3}')
		exp_date=$(date -d"${exp}" "+%d %b %Y")
		...
		echo -e "---------------------"
		echo -e "OpenVPN TCP : 'cat /multivpn/openvpn/client-tcp.ovpn'"
		echo -e "OpenVPN UDP : 'cat /multivpn/openvpn/client-udp.ovpn'"
		echo -e ""
	else
		echo -e ""
		echo -e "User '$user' does not exist."
...
function ovpn-config() {
	...
	1)
		...
		echo -e "--------------------"
		cat /multivpn/openvpn/client-tcp.ovpn
		echo -e ""
		exit
		;;
	2)
		...
		echo -e "--------------------"
		cat /multivpn/openvpn/client-udp.ovpn
		echo -e ""
		exit
		;;
	3)
